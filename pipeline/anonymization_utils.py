...
#
# Modified by Ruben Lucassen:
# - removed main function
# - removed support for Ventana tiffs 
#   (because of undefined variable and not used in this work)

"""
Delete the slide label from an TIF, MRXS, NDPI, or SVS whole slide image.
"""
...
class TiffEntry(object):
    ...
    def overwrite_entry(self, byte_string):
        ...
        # WARNING: Assumes entry contains a string. 
        # May fail if this function is extended to non-string types.
        ...
        if self.type == ASCII:
            # ASCII uses nulls to divide substrings, 
            # so we pad with spaces instead.
...
def do_hamamatsu_ndpi(filename):
    with TiffFile(filename) as fh:
        ...
        # Find and delete macro image and clear any values for NDPI_REFERENCE
        for directory in fh.directories:
            if directory.entries[NDPI_SOURCELENS].value()[0] == -1:
                directory.delete(expected_prefix=JPEG_SOI)
            if NDPI_REFERENCE in directory.entries:
                directory.entries[NDPI_REFERENCE].overwrite_entry(b'')
            if NDPI_BARCODE in directory.entries:
                directory.entries[NDPI_BARCODE].overwrite_entry(b'')
...
def anonymize_non_dicom(filename):
    """ 
    Anonymize a single non-DICOM slide.
    """
